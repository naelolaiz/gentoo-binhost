...
- **`scripts/build.sh` — repair broken NEEDED entries before building.** When a previous chain upgraded `llvm-core/clang` from slot 21 to 22, installed binaries linked against `libclang-cpp.so.21.1` (e.g. `dev-util/mesa_clc`) became unrunnable in the new chain — the next emerge that invoked them failed with `cannot open shared object file` (exit 127), surfaced as `failed in phase 'unknown'`. New `rebuild_broken_libs` step runs `emerge @preserved-rebuild` and `revdep-rebuild --pretend`-driven oneshot rebuilds before the main build, so subsequent emerges see a healthy ELF graph. Honours the same `--max-build-time` deadline as the main build.
